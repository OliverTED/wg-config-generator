...
def main(config: str, privkeys: str):
    ...
    if not fn_config.exists():
        print(f"""Could not find the configuration in '{fn_config}'.

Please assure your local config exists.  See documentation at
https://github.com/OliverTED/wg-config-generator.
""")
        sys.exit(1)

    print(f"reading: {fn_config}")
    config_ = parse_config(fn_config)
    # print(config_)

    fn_config_privkeys = Path(privkeys)
    print(f"trying to read: {fn_config_privkeys}")
    ...
    print(f"writing: {fn_config_privkeys}")
